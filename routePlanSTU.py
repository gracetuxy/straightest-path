...
def good(gst, d, v, w):
    return v not in list(gst.adj[w]) and \
           ...
           graph.edges[v, w, 0]['length'] > 0 and \
           d + graph.edges[v, w, 0]['length'] < goal_dist*1.1
...
def get_bearing_diff(b1,b2):
    ...
    return bdiff if bdiff <= 180 else 360-bdiff
...
def total_elevation_gain(gr,rt):
    ...
    for k in range(len(rt)):
        if k == 0:
            if gr.nodes[rt[k]]['elevation'] > 0:
                eg += gr.nodes[rt[k]]['elevation']
        else:
            if gr.nodes[rt[k]]['elevation'] > gr.nodes[rt[k-1]]['elevation']:
                eg += gr.nodes[rt[k]]['elevation']-gr.nodes[rt[k-1]]['elevation']
    return eg
...
routevertices = [vert[1] for vert in sorted_lst] #list comprehension to assemble the list of vertices in order

# find coordinates of stopping point: last node on the route
endlat, endlon = graph.nodes[routevertices[-1]]['y'], graph.nodes[routevertices[-1]]['x']
...
def shade_given_time(k, n):
    col = colorsys.hsv_to_rgb(k / n, 1, 1)
